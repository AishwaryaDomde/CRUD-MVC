...
Creation Date	- May 7, 2019 12:04:15 PM
Project Name	- MLWB 
Module Name		- 
...
<title>Employee Leave Sanction</title>
...
let global_LeaveSancId  = 0;
let op_flag = "N";
let showForm = false;
let showTable = true;
let formValidator=null,To_apply,applied;
...
$(document).ready(function() {	
	...
	getsetInt();
	validateForm();
	$("#Reco_recomedLeaveDays,#Reco_publicHoliday,#Reco_leaveDays").numeric({decimalPlaces: 2 ,negative: false}).addClass("speech-right");
	$("#Apply_applyLeaveDays,#Apply_publicHoliday,#Apply_leaveDays").numeric({decimalPlaces: 2 ,negative: false}).addClass("speech-right");
	$("#sanc_leaveDays,#sanc_SancLeaveDay").numeric({decimalPlaces: 2 ,negative: false}).addClass("speech-right");
	$("#sanc_publicHoliday").numeric({negative: false}).addClass("speech-right numbersOnly");
	$("#sanc_toDate").datepicker({
	    changeMonth: true,
	    changeYear: true,
	    dateFormat: "dd/mm/yy",
	    minDate:'0'
	 }); 
	$("#sanc_fromDate").datepicker({
	    changeMonth: true,
	    changeYear: true,
	    dateFormat: "dd/mm/yy",
	    minDate:'0'
		...
		success : function(resp) {
			...
			var optionsList='';	
			 ...
			 optionsList=listOfEmpOptions+optionsList+optGrpEnd;
	       	 $("#sanc_reasonCodeId").empty().append(optionsList);
	       	 $('#sanc_reasonCodeId').select2({
	    		    width: '100%',
	    			placeholder:"Select",
	    			allowClear: true,
	...
	$.ajax({  
	    ...
	    success: function(respp){
	    	resp = (respp["helpList"]);
	    	$("#sanc_LeaveName").empty().append("<option></option>");
	    	$.each(resp, function(key,value) {
	    	    if(resp[key].prop8=="Y"){
		    	    nKey = parseInt(key)+1;
					$("#sanc_LeaveName").append('<option value="'+resp[key].prop2+'"> '+resp[key].prop3);
	    	    }
			}); 
	      },
	 });
	$('.select2-container').tooltipster({
	 ...
	 $("#SaveLeaveSanc").on("click",function(){
		 if($("#LeaveSanc").valid()){
			 ...
			 let sanc_Flgchck1="N";      let sanc_Flgchck2 = "N"
			 	...
			 	if($("#sanc_Fhalf").is(":checked")){
			 		sanc_Flgchck1 = "Y";
				}else{
					sanc_Flgchck1= "N";
				}
			 	if($("#sanc_Shalf").is(":checked")){
			 		sanc_Flgchck2 = "Y";
				}else{
					sanc_Flgchck2= "N";
				}
				let postParameters = {
						...
						Reco_recomedLeaveDays	 : $("#Reco_recomedLeaveDays").val(),	
						Reco_publicHoliday		 : $("#Reco_publicHoliday").val(),		
						Reco_leaveDays			 : $("#Reco_leaveDays").val(),			
						Reco_recomendLeaveNameId : $("#Reco_recomendLeaveNameId").val(),
						...
						/*************************************************************/
						sanc_EmpName			: $("#sanc_EmpName").val(),
						sanc_EmpDept			: $("#sanc_EmpDept").val(),
						sanc_Date				: $("#sanc_Date").val(),
						sanc_fromDate			: $("#sanc_fromDate").val(),
						sanc_Fhalf				: sanc_Flgchck1,		
						sanc_toDate				: $("#sanc_toDate").val(), 
						sanc_Shalf				: sanc_Flgchck2,		
						sanc_LeaveDay			: $("#sanc_SancLeaveDay").val(),  
						sanc_publicHoliday		: $("#sanc_publicHoliday").val(),
						sanc_leaveDays			: $("#sanc_leaveDays").val(),  
						sanc_LeaveName			: $("#sanc_LeaveName").val(),  
						sanc_Reason				: $("#sanc_Reason").val(),  
						sanc_leaveSancFlag		: $("#sanc_leaveSancFlag").val(),
						sanc_reasonCodeId		: $("#sanc_reasonCodeId").val(),
						Recom_opFlag	  		: op_flag,
						LeaveId    				: global_LeaveSancId,
						Sanc_userId				: $("#userId").val(),	
						Apply_userId			: $("#Apply_userId").val(),
						Reco_userId				: $("#Reco_userId").val(),		
			...
			if(op_flag=="D"){
				...
				$.ajax({
					url : "${pageContext.request.contextPath}/app/payroll/EmpLeave/LeaveSanction/SaveEmpLeaveSanc",
 	...
 	/****************************************************/	
 	$("#sanc_fromDate ,#sanc_toDate").on("change",function(){
	 	 	var LeaveToDate = $("#sanc_toDate").val();
	 	 	var leaveFromDate = $("#sanc_fromDate").val();
	 		...
	 		$.ajax({  
	 			...
	 			success : function(resp) {
	 				responList =JSON.parse(resp["codeList"]);
	 				$("#sanc_SancLeaveDay").val(responList[0].days_leave);
	 				dochckLeaves()
	 	 		}
	 		}); 	
	 	}	
			$("#sanc_publicHoliday").trigger("change");
	 });
 	/****************************************************/
 	$("input[type=checkbox]").on("change",function(){
		var AplyLeave = $("#sanc_SancLeaveDay").val();
		var totalAplyLeave;
		if($(this).is(":checked")){
			totalAplyLeave =parseFloat(AplyLeave) - Number(0.5);
			$("#sanc_SancLeaveDay").val(totalAplyLeave);
		}else{
			totalAplyLeave =parseFloat(AplyLeave) + Number(0.5);
			$("#sanc_SancLeaveDay").val(totalAplyLeave);
			}
		$("#sanc_publicHoliday").trigger("change");dochckLeaves()
	});
	/***************************************************/
	$("#sanc_publicHoliday").on("change",function(){
		var AplyLeave = $("#sanc_SancLeaveDay").val();
		var Reco_publicHoliday = $("#sanc_publicHoliday").val();
		if(AplyLeave>Reco_publicHoliday){
		var totalAplyLeave =parseFloat(AplyLeave) - Number(Reco_publicHoliday);
		$("#sanc_leaveDays").val(totalAplyLeave);
		}else{
			$("#sanc_publicHoliday").val(0);
			$("#sanc_leaveDays").val($("#sanc_SancLeaveDay").val());
			}dochckLeaves()
	});
	/**********************For Checking whether leaves are available of not*******************************************/
	$("#sanc_LeaveName").on("select2:select",function(){
		let SLeaveName = $(this).val();
		let Tbl_LeaveName,isAvil;
		$('#sanc_OverView tbody>tr>td:nth-child(2).display_hide').each(function() {
			Tbl_LeaveName =$(this).text();
			if(Tbl_LeaveName==SLeaveName){
				isAvil=$(this).nextAll('td').closest(".avil_Leave").text();
				if(isAvil=="0" && SLeaveName!="6"){
					$("#sanc_LeaveName").select2('destroy'); 
					$("#sanc_LeaveName").select2({placeholder:"Select",
			 				width:'100%',
			 				multiple:false,
			 				allowClear:true,					
			 		});
					$("#sanc_LeaveName").find('.select2-container').tooltipster({
			 			    functionInit: function(instance, helper){
			 			        let content = $(helper.origin).closest('.form-group').find('select').attr("title");
			 			        instance.content(content);
			 			    },
					...
					$("select").find('option').each(function(i) {
						$('#sanc_LeaveName option[value='+SLeaveName+']').attr('disabled','disabled');
					});
					$("#sanc_LeaveName").val("").trigger("change");
					$("#sanc_LeaveName").siblings('small').append("Please select another Leave,this leave type not available");
					
				}else{$("#tabLabel3").trigger('click')}
			}
		});
	});
	//*******************************************************************/
	$("#tabLabel3").on("click",function(){
		var sanLeaveId= $("#sanc_LeaveName").val();
// 		var applied;
	    $.ajax({  
		    url:"${pageContext.request.contextPath}/app/common/getCodeList",  
		    data : JSON.stringify({
				"param" : sanLeaveId+"~"+$("#userId").val(),
				"sqlMstId" :7338
			}),
			method : "post",
			dataType : "json",
...
/*************************Document Ready over function starts Reset Settings*************************/
 function validateForm(){
 	formValidator = $("#LeaveSanc").validate({
         rules: {
         	},
         messages: {
          },
 ...
 function dochckLeaves(){
     var sanLeaveId= $("#sanc_LeaveName").val();
	    $('#sanc_OverView tbody>tr>td:nth-child(2).display_hide').each(function() {
	      var  Tbl_LeaveName =$(this).text();
	        if(sanLeaveId==Tbl_LeaveName){
			    To_apply= ($(this).closest("tr").find(".avil_Leave").text()-applied);			    
			}
	    });
	   setTimeout(function(){if($("#sanc_leaveDays").val()>To_apply){
		    $("#sanc_leaveDays").val("")
		  }},10);
 ...
 function toggleTableAndForm(flag, empMstId) {
 	if (showForm) {
 		...
 		if (flag == 'M') {
 			$("#ResetLeaveSanc").hide();
 			$('.formTitle').html('Update Employee Leave Sanction');
 			$('.submitBtnName').html('<i class="fa fa-save"></i> Update Employee Leave Sanction ');
 			resetForm();
 			DisableInputs();
 		...
 		if (flag == 'N') {	
 			$("#ResetLeaveSanc").show();	
 			$('.formTitle').html('Create New Employee Leave Sanction ');
 			$('.submitBtnName').html('<i class="fa fa-save"></i> Save Employee Leave Sanction ');
 			$('.submitBtnName').attr('disabled', 'disabled');
 			resetForm();
 			DisableInputs();
 			GetDetails()
 		...
 		if (flag == 'D') {
 			$("#ResetLeaveSanc").hide();
 			$('.formTitle').html('<i class="fa fa-save"></i> Delete  Employee Leave Sanction');
 			$('.submitBtnName').html('Delete Employee Leave Sanction ');
 			resetForm();
 			EnableInputs();
 			SetApplyRecomdDetails(empMstId);
 			GetDetails();
...
/*********************resetForm**************************************/
 function resetForm(){
	...
	op_flag="N";
	global_LeaveSancId=0;		
	formValidator.resetForm();	
}
/********************************************************************/
 function DisableInputs(){
	 $("#sanc_Date").removeAttr('disabled');
	 $("#sanc_fromDate").removeAttr('disabled');
	 $("#sanc_Fhalf").removeAttr('disabled');
	 $("#sanc_toDate").removeAttr('disabled');
	 $("#sanc_Shalf").removeAttr('disabled');
	 $("#sanc_publicHoliday").removeAttr('disabled');
	 $("#sanc_LeaveName").removeAttr('disabled');
	 $("#sanc_Reason").removeAttr('disabled');
	 $("#sanc_leaveSancFlag").removeAttr('disabled');
	 $("#sanc_reasonCodeId").removeAttr('disabled');
}
/********************************************************************/
 function EnableInputs(){	console.log("Sdf")
	 $("#sanc_Date").attr("disabled","disabled");
	 $("#sanc_fromDate").attr("disabled","disabled");
	 $("#sanc_Fhalf").attr("disabled","disabled");
	 $("#sanc_toDate").attr("disabled","disabled");
	 $("#sanc_Shalf").attr("disabled","disabled");
	 $("#sanc_SancLeaveDay").attr("disabled","disabled");
	 $("#sanc_publicHoliday").attr("disabled","disabled");
	 $("#sanc_leaveDays").attr("disabled","disabled");
	 $("#sanc_LeaveName").attr("disabled","disabled");
	 $("#sanc_Reason").attr("disabled","disabled");
	 $("#sanc_leaveSancFlag").attr("disabled","disabled");
	 $("#sanc_reasonCodeId").attr("disabled","disabled");
}
...
/**************This is used to get Details of Application and Sanction************************/
			...
			$("#EditEmpFamilyDet tbody").empty();
	     	...
	     	$.each(respListSet, function (key,j) {
     			if(respListSet[key]['leave_appli_id']==SectId){
					...
					if(respListSet[key]['sac_first_half']=="YES"){
						$("#sanc_Fhalf").prop('checked', true).val("Y").trigger("change");
					}else{
						if(respListSet[key]['rec_first_half']=="YES"){
							$("#sanc_Fhalf").prop('checked', true).val("Y");
						}else if(respListSet[key]['first_half']=="YES"){
								$("#sanc_Fhalf").prop('checked', true).val("Y");
						}else{
							$("#sanc_Fhalf").prop('checked', false).trigger("change");
						}
					}
					if(respListSet[key]['sac_second_half']=="YES"){
						$("#sanc_Shalf").val("Y");
					}else{
						if(respListSet[key]['rec_second_half']=="YES"){
							$("#sanc_Shalf").val("Y");
						}else if(respListSet[key]['second_half']=="YES"){
								$("#sanc_Shalf").val("Y");
						}else{
							$("#sanc_Shalf").prop('checked', false).trigger("change");
						};
					}
					/***************************/
					...
					/*********************************************/		
					$("#Reco_userId").val(respListSet[key]['recomend_emp_id']);					 
					$("#Reco_EmpName").val(respListSet[key]['recomend_emp_name'])	;				 
					$("#Reco_EmpDepartment").val(respListSet[key]['recomend_dept_name']);							 
					$("#Reco_Date").val(respListSet[key]['recomend_date']);	
					$("#Reco_fromDate").val(respListSet[key]['recomend_from_date']);	
					$("#Reco_toDate").val(respListSet[key]['recomend_to_date']);			
					$("#Reco_recomedLeaveDays").val(respListSet[key]['rec_apply_leave_days']);							 
					$("#Reco_publicHoliday").val(respListSet[key]['rec_public_holiday']);								 
					$("#Reco_recomendLeaveNameId").val(respListSet[key]['recomend_leave_mst_id']);									 
					$("#Reco_recomendLeaveName").val(respListSet[key]['recomend_leave_name']);								 
					$("#Reco_sancReason").val(respListSet[key]['reason']);								 
					$("#Reco_leaveRecomendFlag").val((respListSet[key]['leave_recomend_flag']=="Y")?"NO":"Yes");								
					$("#Reco_reasonCodeId").val(respListSet[key]['reason_code_id']);
					$("#Reco_reasonCodeName").val(respListSet[key]['reason_code_name']);
					$("#Reco_Date").val(respListSet[key]['recomend_date']);	
					$("#Reco_leaveDays").val(respListSet[key]['rec_leave_days']);	
					/**************************************************************/	
					$("#sanc_Date").val(TodaySet);
					$("#sanc_SancLeaveDay").val(respListSet[key]['sac_apply_leave_days']==null?(respListSet[key]['rec_apply_leave_days']):(respListSet[key]['sac_apply_leave_days'])); 
					$("#sanc_publicHoliday").val(respListSet[key]['sac_public_holiday']==null?(respListSet[key]['rec_public_holiday']):(respListSet[key]['sac_public_holiday']));  
					$("#sanc_leaveDays").val(respListSet[key]['sac_leave_days']==null?(respListSet[key]['rec_leave_days']):(respListSet[key]['sac_leave_days']));  
					$("#sanc_LeaveName").val((respListSet[key]['sanction_leave_mst_id'])==null?(respListSet[key]['recomend_leave_mst_id']):(respListSet[key]['sanction_leave_mst_id'])).trigger("change"); 
					$("#sanc_Reason").val(respListSet[key]['reason']);  
					$("#sanc_leaveSancFlag").val(respListSet[key]['leave_sanc_flag']).trigger("change");
					$("#sanc_reasonCodeId").val(respListSet[key]['reason_code_id']);
					if(respListSet[key]['leave_sanc_flag']=="Y"){
						EnableInputs();
					}else{
						DisableInputs();
					}
					$("#sanc_fromDate").val(recomend_from_date).trigger("change");			
					$("#sanc_toDate").val(recomend_to_date).trigger("change"); 
					if($("#sanc_Fhalf").val()=="Y"){
						$("#sanc_Fhalf").prop('checked', true).trigger("change");
					}
					 if($("#sanc_Shalf").val()=="Y"){
						$("#sanc_Shalf").prop('checked', true).trigger("change");
					}
		 		}
			})
		}
...
/*******************This is used for View list Initial****************************************************/
 function LoadListTbl(){
	...
	$.ajax({
	    ...
	    success: function (resp) {
    		...
    		LeaveSancList  =(resp["helpList"]);
        	$.each(LeaveSancList, function (key,i) {
        		keyVal=keyVal+1;        		
 				datacourt.push([keyVal,LeaveSancList[key]['prop4'],LeaveSancList[key]['prop5'],LeaveSancList[key]['prop6'],LeaveSancList[key]['prop7'],LeaveSancList[key]['prop8'],
				'<a href="javascript:;" onclick="toggleTableAndForm(\'M\',\''+LeaveSancList[key]['prop2']+'\')" class="btn btn-primary btn-sm btn-round" title="Edit">'+
	...
	$.ajax({
	   ...
	   success: function(response) {		       	    
		   resp = JSON.parse(response["codeList"]);
	    	$.each(resp, function(key,value) {
      	    		$("#sanc_EmpName").val(resp[key]["name"]);
          			$("#sanc_EmpDept").val(resp[key]["desig_name"]);
 ...
 function GetDetails(){
	...
	$.ajax({ 
		...
		contentType : "application/json",
		success : function(res) {
		if ($.fn.DataTable.isDataTable('#sanc_OverView')) {
                $('#sanc_OverView').DataTable().destroy();
            }
            $("#sanc_OverView tbody").empty();
        	let keyVal=0;	let datacourt=[];
	...
	<div class="col-lg-12">
		<div class="panel newPanel">
			<div class="panel-heading">
				...
				<h3 class="panel-title">Employee Leave Sanction</h3>
			</div>
...
<div class="row showFormDiv">
	<div class="col-lg-12">
		<div class="panel-heading">
			...
			<h3 class="panel-title formTitle"> Create New Employee Leave Sanction </h3>
		</div>
		<div class="panel-body">
			<div class="row" id="tab1" data-role="tab">
				<ul class="nav nav-tabs">
					...
					<li class="">
					<a href="#tabContent3" data-toggle="tab" id="tabLabel3"><spring:message code="label.hrLeaveSanction"></spring:message></a>
					</li>
				</ul>
				<br>
				<div class="tab-content">
				  	<div class="tab-pane fade active in" id="tabContent1">
						<div class="panel-body">
							<div class="row">
								...
								<div class="col-sm-3">
									<div class="form-group">
										<label for="deptId" class="control-label LblImp">
							...
							<div class="row">
								...
								<div class="col-sm-3">
									<div class="form-group">
										<label for="recoEmpName LblImp" class="control-label LblImp">
							...
							<div class="row">
								<div class="col-sm-6">
									<div class="form-group">
										<label for="leaveDaysAddress"  class="LblImp control-label">
					...
					<div class="tab-pane fade" id="tabContent2">
						<div class="panel-body">	
							<div class="row">
								<div class="col-sm-3">
									<div class="form-group">
										...
										<input name="Reco_userId" type="hidden" class="form-control" data-toggle="tooltip"  size="60" maxlength="200"  id="Reco_userId"  disabled>
										<input name="recoEmpName" type="text" class="form-control " data-toggle="tooltip"
										 placeholder="Enter <spring:message code="label.recoEmpName"></spring:message>"  title="Enter Party Filling Case" 
										  size="60" maxlength="200"  id="Reco_EmpName"  disabled>
									</div>
								</div>
								<div class="col-sm-3">
									<div class="form-group">
										<label for="recoEmpDepartment" class="control-label LblImp">
											<spring:message code="label.recoEmpDepartment"></spring:message>
										</label>
										<input name="recoEmpDepartment" type="text" class="form-control " data-toggle="tooltip"
								...
								<div class="col-sm-2">
									<div class="form-group">
										<div class="form-group has-feedback">	
											<label for="recomendDate" class="control-label">
												<spring:message code="label.recomendDate"></spring:message></label>
											<input name="recomendDate" type="text" class="form-control" data-toggle="tooltip" id="Reco_Date" disabled
								...
								<div class="col-sm-2">
									<div class="form-group">
										<div class="form-group has-feedback">	
											...
											<input name="fromDate" type="text" class="form-control " data-toggle="tooltip" id="Reco_fromDate" disabled
											 placeholder="dd/mm/yy" maxlength="10" style="text-align: center;"  title="Enter <spring:message code="label.fromDate"></spring:message>">
											<span class="fa fa-calendar fa-lg form-control-feedback"></span>
											<input type="checkbox" value="N" id="Reco_Fhalf" disabled>Half Day
								...
								<div class="col-sm-2">
									<div class="form-group">
										<div class="form-group has-feedback">	
											...
											<input name="toDate" type="text" class="form-control " data-toggle="tooltip" id="Reco_toDate" disabled
											 placeholder="dd/mm/yy" maxlength="10" style="text-align: center;"  title="Enter <spring:message code="label.toDate"></spring:message>">
											<span class="fa fa-calendar fa-lg form-control-feedback"></span>
											<input type="checkbox" value="N" id="Reco_Shalf" disabled>Half Day
							...
							<div class="row">
								<div class="col-sm-3">
									<div class="form-group">
										...
										<input name="RecLeaveDays" type="text" class="form-control " data-toggle="tooltip" disabled
								...
								<div class="col-sm-3">
									<div class="form-group">
										...
										<input name="publicHoliday" type="text" class="form-control " data-toggle="tooltip" disabled
								...
								<div class="col-sm-2">
									<div class="form-group">
										...
										<input name="leaveDays" type="text" class="form-control " data-toggle="tooltip" disabled
								...
								<div class="col-sm-2">
									<div class="form-group">
										...
										<input type="hidden" name="recomendLeaveName" class="form-control " data-toggle="tooltip"
										 title="Enter Party Filling Case"  id="Reco_recomendLeaveNameId" disabled >
										<input type="text" name="recomendLeaveName" class="form-control " data-toggle="tooltip"
										 title="Enter Party Filling Case"  id="Reco_recomendLeaveName" disabled >
									</div>
								</div>
								<div class="col-sm-2">
									<div class="form-group">
										<label for="leaveRecomendFlag" class="control-label">
											<spring:message code="label.leaveRecomendFlag"></spring:message></label>
										<input name="leaveRecomendFlag" class="form-control Advocate" data-toggle="tooltip"  disabled
							...
							<div class="row">
								<div class="col-sm-6">
									<div class="form-group">
										<label for="sancReason"  class="LblImp control-label">
											<spring:message code="label.sancReason"></spring:message></label>
										<textarea name="sancReason" class="form-control " data-toggle="tooltip" disabled
								...
								<div class="col-sm-2">
									<div class="form-group">
										<label for="reasonCodeId" class="control-label">
											<spring:message code="label.reasonCodeId"></spring:message></label>
											<input type="hidden" name="Reco_reasonCodeId" class="form-control Advocate" data-toggle="tooltip"  
											title="Select <spring:message code="label.reasonCodeId"></spring:message>"  id="Reco_reasonCodeId">
											<input type="text" name="reasonCodeNameAply" class="form-control Advocate" data-toggle="tooltip"  disabled
											title="Select <spring:message code="label.reasonCodeId"></spring:message>"  id="Reco_reasonCodeName">
									</div>
								</div>
							</div>	
						</div>
					</div>
					<!-- -----------------------Third Tab goes from here------------------- -->							
					<div class="tab-pane fade" id="tabContent3">
						<div class="row">
							<form id="LeaveSanc">	
								...
								<div class="row">
									<div class="col-sm-12">
										<div class="form-group">
											<label for="sancEmpName" class="control-label LblImp">
												<spring:message code="label.sancEmpName"></spring:message>
											</label>
											<input name="sancEmpName" type="text" class="form-control mandatory" data-toggle="tooltip"
											 placeholder="Enter <spring:message code="label.sancEmpName"></spring:message>"  title="Enter Party Filling Case" 
											  size="60" maxlength="200"  id="sanc_EmpName" required disabled>
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											<label for="sanctEmpDept" class="control-label LblImp">
												<spring:message code="label.sanctEmpDept"></spring:message>
											</label>
											<input name="sanctEmpDept" type="text" class="form-control mandatory" data-toggle="tooltip"
											 placeholder="Enter <spring:message code="label.sanctEmpDept"></spring:message>"  title="Enter Party Filling Case" 
											  size="60" maxlength="200"  id="sanc_EmpDept" required disabled >
											  <small style="color: red"></small>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											<div class="form-group has-feedback">	
												<label for="sanctionDate" class="control-label">
													<spring:message code="label.sanctionDate"></spring:message></label>
												<input name="sanctionDate" type="text" class="form-control" data-toggle="tooltip" id="sanc_Date"
												 placeholder="dd/mm/yy" maxlength="10" style="text-align: center;"  title="Enter <spring:message code="label.sanctionDate"></spring:message>">
												<span class="fa fa-calendar fa-lg form-control-feedback"></span>
												<small style="color: red"></small>
											</div>
										</div>
								...
								<!------------------------------------------------------------>
									...
									<div class="col-sm-6">
										<div class="form-group">
											<div class="form-group has-feedback">	
												<label for="fromDate" class="control-label LblImp">
													<spring:message code="label.fromDate"></spring:message></label>
												<input name="fromDate" type="text" class="form-control mandatory" data-toggle="tooltip" id="sanc_fromDate" readonly
												 placeholder="dd/mm/yy" maxlength="10" style="text-align: center;" required title="Enter <spring:message code="label.fromDate"></spring:message>">
												<span class="fa fa-calendar fa-lg form-control-feedback"></span>
												<input type="checkbox" value="N" id="sanc_Fhalf">Half Day
												<small style="color: red"></small>
											</div>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											<div class="form-group has-feedback">	
												<label for="toDate" class="control-label LblImp">
													<spring:message code="label.toDate"></spring:message></label>
												<input name="toDate" type="text" class="form-control mandatory" data-toggle="tooltip" id="sanc_toDate" readonly
												 placeholder="dd/mm/yy" maxlength="10" style="text-align: center;" required title="Enter <spring:message code="label.toDate"></spring:message>">
												<span class="fa fa-calendar fa-lg form-control-feedback"></span>
												<input type="checkbox" value="N" id="sanc_Shalf">Half Day
												<small style="color: red"></small>
											</div>
										</div>
									</div>
									...
									<div class="col-sm-6">
										<div class="form-group">
											<label for="sancLeaveDays" class="control-label LblImp">
												<spring:message code="label.sancLeaveDays"></spring:message>
											</label>
											<input name="sancLeaveDays" type="text" class="form-control mandatory" data-toggle="tooltip"
											 placeholder="Enter <spring:message code="label.sancLeaveDays"></spring:message>"  title="Enter Party Filling Case" 
											  size="60" maxlength="200"  id="sanc_SancLeaveDay" required disabled>
											  <small style="color: red"></small>
										</div>											
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											...
											<input name="publicHoliday" type="text" class="form-control mandatory" data-toggle="tooltip"
											 placeholder="Enter <spring:message code="label.publicHoliday"></spring:message>"  title="Enter Party Filling Case" 
											  size="60" maxlength="200"  id="sanc_publicHoliday" required >
											  <small style="color: red"></small>
										</div>																								
									</div>
									</div>
									...
									<div class="col-sm-6">
										<div class="form-group">
											...
											<input name="leaveDays" type="text" class="form-control mandatory" data-toggle="tooltip"
											 placeholder="Enter <spring:message code="label.leaveDays"></spring:message>"  title="Enter Party Filling Case" 
											  size="60" maxlength="200"  id="sanc_leaveDays" required disabled>
											  <small style="color: red"></small>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="form-group">
											<label for="sanctLeaveName" class="control-label LblImp">
												<spring:message code="label.sanctLeaveName"></spring:message>
											</label>
											
											<select name="sanctLeaveName" class="form-control mandatory" data-toggle="tooltip"
											title="Enter Party Filling Case" id="sanc_LeaveName" required ></select>
											  <small style="color: red"></small>
										</div>
									</div>
									</div>
									...
									<div class="col-sm-12">
										<div class="form-group">
											<label for="sancReason"  class="LblImp control-label">
												<spring:message code="label.sancReason"></spring:message></label>
											<textarea name="sancReason" class="form-control mandatory" data-toggle="tooltip"
											 placeholder="Enter Subject Matter"  title="Enter <spring:message code="label.sancReason"></spring:message>" required
											 rows="3" maxlength="2000"  id="sanc_Reason"  style='resize: vertical;'></textarea>
										<small style="color: red"></small>
										</div>										
									</div>
									</div>
									...
									<div class="col-sm-6">
										<div class="form-group">
											<label for="leaveSancFlag" class="control-label">
												<spring:message code="label.leaveSancFlag"></spring:message></label>
											<select name="leaveSancFlag" class="form-control Advocate" data-toggle="tooltip"  
											  title="Select <spring:message code="label.leaveSancFlag"></spring:message>"  id="sanc_leaveSancFlag">
									...
									<div class="col-sm-6">
										<div class="form-group">
											<label for="reasonCodeId" class="control-label">
												<spring:message code="label.reasonCodeId"></spring:message></label>
											<select name="reasonCodeId" class="form-control Advocate" data-toggle="tooltip"  
											  title="Select <spring:message code="label.reasonCodeId"></spring:message>"  id="sanc_reasonCodeId">
											</select>
											<small style="color: red"></small>
										</div>
									</div>
								...
								<!-- -------------------------------------------------------------------------------------------------- -->
								<div class="col-sm-7">
									<table class="table table-striped table-bordered table-hover locationMasterTable table-responsive" id="sanc_OverView">
									<thead>
										<tr>
											<th>Sr. No.			</th>
											<th>Sr. No.ID			</th>
						...
						<div class="col-sm-12">
						<hr>
							<button  class="btn btn-primary submitBtnName" type="button" id="SaveLeaveSanc">
							 <i class="fa fa-save"></i>Save Employee Leave Sanction
							</button>
							&nbsp;
							<button type="button" class="btn resetBtnName btn-danger" id="ResetLeaveSanc" onclick="resetForm()">
							Reset
							</button>
						</div>
					</div>	
